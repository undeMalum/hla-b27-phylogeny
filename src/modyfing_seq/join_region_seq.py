...
def join_allele_regions(out_dir: Path, file_format: str):
    with open(out_dir, "w") as joined_seq, open(ASIAN_SEQ_FILE, "r") as asian_seq_it,\
        ...
        asian_seq_list_modified_header = modify_headings.modify_headings(asian_seq_list)

        eu_seq_list = list(Bio.SeqIO.parse(eu_seq_it, file_format))
        eu_seq_list_modified_header = modify_headings.modify_headings(eu_seq_list)
...
def main():
    ...
    join_allele_regions(EU_SEQ_HEAD_MODIFIED_FILE, chosen_file_format)
    with open(EU_SEQ_HEAD_MODIFIED_FILE) as join_seq:
